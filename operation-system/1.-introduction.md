...
An operating system is software that manages a computer’s hardware, acts as an intermediary between the user and the hardware. A more common definition, and the one that we usually follow, is that the operating system is the one program running __at all times on the computer—usually called the **kernel**. Along with the kernel, there are two other types of programs: **system programs**, which are associated with the operating system but are not necessarily part of the kernel, and **application programs**, which include all programs not associated with the operation of the system.
...
Interrupts are used throughout modern operating systems to handle asynchronous **events** \(and for other purposes. To start an I/O operation, the device driver loads the appropriate registers \(寄存器\) in the device controller. The controller starts the transfer of data from the device to its local buffer. Once the transfer of data is complete, the device controller **informs** the device driver that it has finished its operation, via an **interrupt**.
...
We say that the device controller _raises_ an interrupt by asserting a signal on the interrupt request line, the CPU catches the interrupt and _dispatches_ it to the **interrupt handler**, and the handler _clears_ the interrupt by servicing the device.
